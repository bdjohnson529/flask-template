...
## Deployment
Build is tested on a Windows 10 x64 computer. Commands should be run from the `Command Prompt` or the `Anaconda Prompt`. Commands may not work on `Windows PowerShell` or `Anaconda Powershell Prompt`. First set up your virtual environment:
```
python -m venv env
